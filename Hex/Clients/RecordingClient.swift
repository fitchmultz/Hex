...
actor RecordingClientLive {
    ...
    private func setOutputMute(deviceID: AudioDeviceID, muted: Bool) -> Bool {
        ...
        // Try main (virtual master) element first
        var mainAddr = AudioObjectPropertyAddress(
            mSelector: kAudioDevicePropertyMute,
            mScope: kAudioDevicePropertyScopeOutput,
            mElement: kAudioObjectPropertyElementMain
        )
        if AudioObjectHasProperty(deviceID, &mainAddr) {
            let status = AudioObjectSetPropertyData(deviceID, &mainAddr, 0, nil, size, &value)
