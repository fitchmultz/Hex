...
private enum CuratedModelLoader {
    static func load() -> [CuratedModelInfo] {
        ...
        do { return try JSONDecoder().decode([CuratedModelInfo].self, from: Data(contentsOf: url)) } catch { assertionFailure("Failed to decode models.json – \(error)"); return [] }
    }
}
    ...
    public var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            case let .modelsLoaded(recommended, available):
                ...
                for idx in curated.indices {
                    curated[idx].isDownloaded = available.first { $0.name == curated[idx].internalName }?.isDownloaded ?? false
                }
        ...
        Reduce { state, action in
            ...
            case .downloadSelectedModel:
                ...
                return .run { [downloadID, modelName] send in
                    do {
                        try await transcription.downloadModel(modelName) { progress in
        ...
        Reduce { state, action in
            ...
            case .deleteSelectedModel:
                ...
                return .run { send in
                    do {
                        try await transcription.deleteModel(modelName)
